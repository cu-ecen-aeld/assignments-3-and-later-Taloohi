...
if (($# != 1))
then
    echo "Missing/incorrect output directory argument"
    echo "Defaulting to /tmp/aeld"
    OUTDIR=/tmp/aeld
else
    echo "output directory entered: $1"
    OUTDIR=$1
fi
...
if [ -d $OUTDIR ]
then
    echo "Path $OUTDIR already exists"
    echo
else
    echo "Attempting to create directory $OUTDIR"
    mkdir -p $OUTDIR
fi

if [ -d $OUTDIR ]
then
    echo "Path $OUTDIR successfuly created"
    echo
else
...
cd ${OUTDIR}/rootfs
mkdir -p bin dev etc home lib lib64 lib64 proc sbin sys tmp usr var
